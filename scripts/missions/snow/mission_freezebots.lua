...
Mission_FreezeBots = Mission_Infinite:new{
	SpawnStartMod = -2,
	SpawnMod = 1,
	Objectives = Objective("Geler et défendre les deux robots",2),
	Bots = nil,
	UseBonus = false,
}	
...
function Mission_FreezeBots:UpdateObjectives()
	if self:CountDead() == 2 then
		Game:AddObjective("Geler et défendre \nles deux robots", OBJ_FAILED, REWARD_REP, 2)
		Game:AddObjective("Geler et défendre le robot restant", OBJ_FAILED, REWARD_REP, 1)
	elseif self:CountDead() == 1 then
		Game:AddObjective("Geler et défendre \nles deux robots", OBJ_FAILED, REWARD_REP, 2)
		local status = self:CountFrozen() == 1 and OBJ_COMPLETE or OBJ_STANDARD
		Game:AddObjective("Geler et défendre le robot restant", status, REWARD_REP, 1)
	else
