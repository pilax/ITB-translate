Mission_BotDefense = Mission_Infinite:new{
	RobotStart = 2,
	Bots = nil,
	Objectives = Objective("Défendre les Robots",2,2),
	UseBonus = false
}
...
function Mission_BotDefense:GetCompletedObjectives()
	if self:CountBots() == 1 then
		return Objective("Défendre les Robots (Un perdu)",1,2)
...
function Mission_BotDefense:UpdateObjectives()
	...
	if status1 == OBJ_FAILED then
		...
		Game:AddObjective("Défendre le robot restant", status2, REWARD_REP, 1)
	end
end