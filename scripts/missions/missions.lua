...
function Mission:FlyingSpawns(origin, count, pawn, projectile_info, exclude)
	...
	while i <= zone:size() do
		if not Board:IsSafe(zone:index(i)) or dont_use[hash(zone:index(i))] or Board:IsEdge(zone:index(i)) then
			zone:erase(i)
		else
			i = i + 1
		end
...
function Mission:GetCeoBriefing(ceo)
	...
	if text == "" then
		text = "Le PDG vous a envoyé un message concernant la mission. Il est très positif et enthousiaste, et vous donne sa vision monde ainsi que son opinion sur la mission que vous êtes sur le point d'entreprendre."
	end
...
function Mission:GetBonusObjective(objective)
	...
	elseif objective == BONUS_MECHS then
		return Objective("Encaisser moins de 4 points de dégâts",1)
	...
	elseif objective == BONUS_ASSET then
		local asset = _G[self.AssetId]
		local text = "Protéger "..asset.Name
...
function Mission:GetRetreatEvent()
	return CreateChoiceEvent(
		{ 
			Text = "Vous avez déserté le champ de bataille, laissant des provisions et des civils derrière vous.",
			structure = "destroy"
		}
	)
end
...
function Mission:GetDeadEvent()
	return CreateChoiceEvent(
		{ 
			Text = "Tous vos Mechs ont été désactivés. Une équipe d'évacuation arrivera sous peu pour récupérer vos pilotes et leurs débris.",
			structure = "destroy"
		}
	)
end
...
--generic missions like "Appui aérien" where it could work 
